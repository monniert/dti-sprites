...
class HtmlImagesPageGenerator:
    ...
    def run(self):
        ...
        directories = sorted(set(map(lambda p: p.parent, get_files_from_dir(
            self.input_dir, INPUT_EXTENSIONS, recursive=True))))
        ...
        for d in directories:
            ...
            file.write('\t\t<h2>{}</h2>\n'.format(subtitle))
            file.write('\t<tr>\n')
            img_files = get_files_from_dir(d, valid_extensions=INPUT_EXTENSIONS, sort=True)
